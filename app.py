...
with st.sidebar.expander("🔄 Account & Entry", expanded=True):
    ...
    default_pl = float(settings.get(f"last_pl_{account}", 0.0))
...
# ─── Heartbeat Chart with Moving Average ───────────────────────
st.subheader("Balance Over Time (Heartbeat + MA)")
x = df_acc["Date"]
y = df_acc["Balance"]

# select MA window
ma_win = st.selectbox("Moving-Average Window (days)", MA_WINDOWS, index=0)
y_ma = y.rolling(window=ma_win, min_periods=1).mean()
...
fig = go.Figure(
    data=[
        ...
        go.Scatter(x=x, y=y, mode="lines+markers",
                   ...
                   marker=dict(size=8, color=neon),
                   name="Balance"),
        # moving average line
        go.Scatter(x=x, y=y_ma, mode="lines",
                   line=dict(color="#FFA500", width=2, dash="dash"),
                   name=f"{ma_win}-day MA")
    ],
    layout=go.Layout(
        xaxis=dict(title="Date", tickformat="%b %d", color=neon),
        yaxis=dict(title="Balance ($)", color=neon),
    ...
    frames=[
        go.Frame(data=[go.Scatter(
            ...
            mode="markers", marker=dict(size=16, color="#FF0055", symbol="heart")
        )]),
        go.Frame(data=[go.Scatter(
            x=[x.iat[last_idx]], y=[y.iat[last_idx]],
            mode="markers", marker=dict(size=8, color=neon, symbol="circle")
        )])
    ]
)
