...
def init_account_state(account: str, start: float = 1000.0, target: float = 2000.0):
    st.session_state.setdefault(f"start_balance_{account}", start)
    st.session_state.setdefault(f"profit_target_{account}", target)

for acct in ACCOUNTS:
    init_account_state(acct)
...
if st.sidebar.button("Undo Last Entry"):
    ...
    if not df_acc.empty:
        ...
        df_all.to_csv(CSV_FILE, index=False)
        st.sidebar.success("Last entry removed for {}.".format(account))
    else:
        st.sidebar.warning("Nothing to undo for this account.")

st.sidebar.subheader("Reset")
st.sidebar.markdown("This will permanently delete all data and reset account balances.")
...
if st.session_state.get("notification"):
    st.info(st.session_state.pop("notification"))
