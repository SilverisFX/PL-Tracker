...
with st.sidebar.expander("⚙️ Settings", expanded=False):
    start_balance_input = st.number_input(
        "Starting Balance",
    ...
    # update both session and settings
    st.session_state[f"start_balance_{account}"] = start_balance_input
    st.session_state[f"profit_target_{account}"] = profit_target_input
    settings[f"start_balance_{account}"] = start_balance_input
    settings[f"profit_target_{account}"] = profit_target_input
...
start_balance = st.session_state[f"start_balance_{account}"]
profit_target = st.session_state[f"profit_target_{account}"]
...
c1.metric("Start", f"${start_balance:,.2f}")
c2.metric("Current", f"${current_balance:,.2f}", delta=f"{pct_gain:+.2f}%")
c3.metric("Progress", f"{progress_pct*100:.1f}%", delta=f"${current_balance - start_balance:+.2f}")
st.markdown("</div>", unsafe_allow_html=True)

# ─── Animated Progress Bar ──────────────────────────────────────
st.markdown(f"""
<style>
.progress-container {{ background-color:#222; border-radius:12px; height:25px; width:100%;
  box-shadow:inset 0 0 4px #444; margin-top:10px; }}
.progress-bar {{ height:100%; width:{progress_pct*100:.1f}%;
  background:linear-gradient(90deg,#00ccff,#3366ff);
  border-radius:12px; box-shadow:0 0 10px #00ccff;
  transition:width 1s ease-in-out; }}
.progress-text {{ font-weight:bold; text-align:right; color:#ccc; padding-top:5px; }}
</style>
